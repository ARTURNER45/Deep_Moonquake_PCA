#doesnt work for stacks
import obspy
import numpy as np
import insight_data as idat
...
class dstack:
    ...
    def read_moon_stacks(self,tgrab=[-500,800],normdata=False):

        # read the data
        st=adat.read_stacks(chans=['MHX','MHY'],catalog='bulow',stats=['s15','s16'])
        adat.write_stack_info(catalog='bulow')
        adat.read_stack_tmax(st,pk='t4') #I have changed minqual to 0 
        #adat.read_stack_picks(st,pk='t3',remnopick=True)

        # which nests, stations, channels
        iev = np.arange(0,len(st))
        #evs,iev=np.unique([tr.stats.location for tr in st],return_inverse=True)
        ...
        self.Nev=len(self.nests)

        # length of window to read
        ndat=int(st[0].stats.npts)
        self.ndata=ndat
        self.data=np.ndarray([ndat,self.Nev,self.Nchan,self.Nstat])
        ...
        for k in range(0,len(st)):
            ...
            #self.data.mask[:,iev[k],ichn[k],istn[k]]=False
        ...
        self.ident_okay_events(rembad=True)
        
    def read_single_moonquakes_2(self,lis,normdata=True,tgrab=[-300,600]):
    #def read_single_moonquakes_2(self,lis,normdata=True,tgrab=[-900,900]):
        # read the data
        #adat.read_corr_picks(lis='saved_picks2.txt') 
        st=adat.read_single_moonquakes(lis=lis)
        #st=adat.read_single_moonquakes(lis='/home/users/lina3509/Documents/Planets/S12new_flat_pickes.txt')
        #adat.write_single_moonquakes_info(catalog='moonquake_trial')
        ##print(st)
        #adat.read_stack_tmax(st,pk='t4') #I have changed minqual to 0 
        #adat.read_stack_picks(st,pk='t3',remnopick=True)/home/users/lina3509/Documents/Apollo/A01_moonquakes/lists/S12_MHN_peaked.txt
       

        # which nests, stations, channels
        #evs,iev=([tr.stats.starttime for tr in st],return_inverse=True)
        #evs,iev=np.unique([tr.stats.starttime for tr in st],return_inverse=True)
        #print(iev)
        iev = np.arange(0,len(st))
        print(iev)
        stns,istn=np.unique([tr.stats.station for tr in st],return_inverse=True)
        ...
        self.stations=stns
        self.channels=chns
        #tgrab = [st[0].stats.starttime,st[0].stats.endtime]
	...
	#for bootstrapping may not be induvidual events -- events can be repeated 
       	self.Nev = len(st)
       	#print('NEv')
        #print(self.Nev)
        ...
        for k in range(0,len(st)):
            ...
            tr.trim(starttime=tref+tgrab[0], # this is where I could do the grab with the correlated region, to make sure that the shift doesnt have to be too big. 
            endtime=tref+tgrab[1]+3*tr.stats.delta,pad=True)
            data=tr.data[0:ndat]
            #plt.plot(data)
            # normalise if desired
    ...
    def filterdata(self):
        ...
        # msk=np.sum(self.data.mask,axis=0,keepdims=True)>0
        # msk=msk.reshape(list(msk.shape)+[1])
        # msk=np.repeat(msk,self.fdata.shape[0],0)
        # msk=np.repeat(msk,self.fdata.shape[4],4)
        # self.fdata=np.ma.masked_array(self.fdata,mask=msk)
    ...
    def initpars(self):
        ...
        # and indices
        self.ishfs = general.closest(self.tdata,self.tshfs.flatten()
                                     -self.twin/2.)
    ...
    def initsearch(self,method='DA',tshfmax=None):
        ...
        if method=='Nelder-Mead':
            ...
            self.TEMPRANGE = [1000]
            self.ref_misfit = -0.8
        #elif method in ['iterative_grid_fine_tune']:
        #elif method in ['iterative_grid_fine_tune']:
            #load best shifts from xcorr 
            #best_ishifts = np.load('ishifts_longer_real.npy')

            #convert ishifts from Xcorr 
            #best_tshifts = best_ishifts.flatten() * self.delta
            #self.searchfun = optimize.brute
            #self.method = 'iterative_grid_fine_tune'
            #self.opttype = 'iterative_grid_fine_tune'
            #search_range = 2.5
            #self.dt = 0.1
            #self.number = (2.5 * 2)/self.dt #something to do with this going wrong I think.
            #self.paramiter_search_range = np.zeros(shape=(int(self.number),self.Nev))

            #for i in range(0,self.Nev):
                #self.paramiter_search_range[:,i] = np.arange(best_tshifts[i] - search_range,best_tshifts[i] + search_range,self.dt)
            #self.Number_of_iterations = 150
            #self.misfit_store = np.zeros(self.Number_of_iterations)
            #self.TEMPRANGE = [1000]
            #self.ref_misfit = -0.8
    ...
    def findtshf(self):
        ...
        #elif self.opttype in ['iterative_grid_fine_tune']:
            #for TEMP in self.TEMPRANGE:
               ##print(TEMP)
               #for k in range(0,self.Number_of_iterations):
                   #self.k = k
                   ##print(k)
                   #for i in range(0,self.Nev):
                       #self.event_iterable = i 
                       #misfit_storage = []
                       #P_storage = [] 
                       #frc_store = []
                       #P_hat =np.zeros(len(self.paramiter_search_range[i]))
                       #for j in range(0,len(self.paramiter_search_range[i])):
                           #misfit = (self.calcmisfit_loop(self.paramiter_search_range[i][j]))
                           #misfit_storage.append(misfit)
                       #ref_misfit = np.median(misfit_storage) - 2*stats.median_absolute_deviation(misfit_storage)
                       #P_hat = TEMP *(1 - np.exp(-(misfit_storage - ref_misfit)/((np.min(misfit_storage)-ref_misfit))))
                       
                       #xm=np.ma.masked_less(P_hat, 0)
                       #P_hat = xm.filled(0)
                       #cum_prob = np.cumsum(P_hat)
                       #irand = np.random.rand(1)[0]*cum_prob[-1]
                       #self.x[i]= np.interp(irand,cum_prob,self.paramiter_search_range[i])  
                       ##print(self.x)
                       ##print(np.min(misfit_storage))
                       #set the time shifts
                       #self.x2tshfs(self.x)    
                       #self.replaceadata(iev=self.iev,ichan=self.ichan,istat=self.istat,ifreq=self.ifreq)
                       #fig, axs = plt.subplots(4, 1)
                       #axs[0].plot(self.paramiter_search_range[i],misfit_storage)
                       #axs[0].set_xlabel('timeshift (s)')
                       #axs[0].set_ylabel('misfit ')           
                       #axs[1].plot(self.paramiter_search_range[i],P_hat)
                       #axs[1].plot(np.repeat(self.x[i],len(P_hat)),P_hat,'orange')
                       #axs[1].set_xlabel('timeshift (s)')
                       #axs[1].set_ylabel('likelyhood ')
                    #axs[0].set_ylim([0,1])
                       #axs[1].grid(True)
                       #a#xs[2].plot(self.paramiter_search_range[i],cum_prob)
                       #axs[2].plot(np.repeat(self.x[i],len(cum_prob)),cum_prob,'orange')
                       #axs[2].set_xlabel('timeshift (s)')
                       #axs[2].set_ylabel('cum prob ')
                       #axs[3].plot(self.adata[:,0,0,0],'plum')
                       #axs[3].plot(self.adata[:,1,0,0],'cornflowerblue')
                       #axs[3].plot(self.adata[:,2,0,0],'teal')
                       #axs[3].plot(self.adata[:,3,0,0],'silver')
                       #axs[3].plot(self.adata[:,4,0,0],'bisque')
                       #axs[3].plot(self.adata[:,5,0,0],'yellowgreen')
                       #axs[3].plot(self.adata[:,6,0,0],'olive')
                       #axs[3].plot(self.adata[:,7,0,0],'darkseagreen')
                       #axs[3].plot(self.adata[:,8,0,0],'tomato')
                       #axs[3].plot(self.adata[:,9,0,0],'lightcoral')
                       #axs[3].plot(self.adata[:,i,0,0],'red')
                       #axs[3].set_xlabel('time index')
                       #axs[3].set_ylabel('amplitude ')
                       #plt.title('Event: '+str(j)+' iteration:'+str(k)+'temp'+str(TEMP)+'fine tune')
                       #fig.tight_layout()
                       #plt.savefig('/home/users/lina3509/Documents/Planets/misfit_timeshift_plots/iteration_'+str(k)+",_event_"+str(i)+"fine_tune.png")
                       #plt.close()
                       #plt.show()
    ...
    def iterative_grid_fine_tune(self,search_range =5, temp = 1000, Niter = 100):
        ...
        for k in range(0,Niter):
            ...
            for i in range(1,self.Nev):
                ...
                #if k == Niter-1: 
                        #plt.plot(ev_search_range,misfit)
                        #plt.savefig('misfit'+str(i)+'.png')
                        #plt.close()
                #plt.show()
                #3ref_misfit = np.median(misfit) - 2*stats.median_absolute_deviation(misfit)	
                #3P_hat = temp *(1 - np.exp(-(misfit - ref_misfit)/((np.min(misfit)-ref_misfit))))
                #xm=np.ma.masked_less(P_hat, 0)
                #P_hat = xm.filled(0)
                #cum_prob = np.cumsum(P_hat)
                #irand = np.random.rand(1)[0]*cum_prob[-1]
                #self.ishfs[i]= np.interp(irand,cum_prob,self.paramiter_search_range[i])  
    ...
    def plotU(self,plotsynth=None,plotdata=False,Ncmp=None,ichan=None,istat=None,ifreq=None):
        ...
        i1=general.closest(self.tdata,tvl)[0]
        ...
        for k in range(0,Ncmp):
            for kc in range(0,N2):
                ...
                p[k,kc].set_xlim(general.minmax(tvl))
    ...
    def x2tshfs(self,x):
        ...
        # closest indices
        self.ishfs = general.closest(self.tdata,x-self.twin/2.)
        ##print(self.ishfs)
    ...
    def grabdata(self,ishfs=None,iev=None,ichan=None,istat=None,ifreq=None):
        ...
        # # sort out the indices - I did not comment this out that was done already 
         #if ichan is None:
             #ichan=np.arange(0,self.Nchan)
         #ichan=np.atleast_1d(ichan)
         #if istat is None:
         ...
         #istat=np.atleast_1d(istat)
         #if iev is None:
             #iev=np.arange(0,self.Nev)
         #iev=np.atleast_1d(iev)
         #if ishfs is None:
             #ishfs=np.arange(0,iev.size)
         #ishfs=np.atleast_1d(ishfs)
         #if ifreq is None:
             #ifreq=np.arange(0,self.Nfreq)
         #ifreq=np.atleast_1d(ifreq)
    ...
    def datafrom2d(self,cdata):
        ...
        # # the shape
        # shp=[self.nwin,1,self.Nchan,self.Nstat,self.Nfreq]
        # shp[1]=int(np.round(cdata.shape[1]/np.prod(shp[2:])))

        # # reshape
        # edata=cdata.reshape(shp)

        return edata
   #def removemean(self,cdata):
    ...
    def calcperclow_saved(self):
        ...
        #np.save('pca_DA_ishfs.npy',self.ishfs)
        #np.save('pca_two_tshfts.npy',self.x)
    ...
    def calcperclow_saved_cmp(self,flatten_tshfts):
        ...
        # closest indices
        #self.ishfs = general.closest(self.tdata,flatten_tshfts
                                    # - self.twin/2.)
        #self.ishfs = self.ishfs.reshape([self.Nev,self.Nstat,self.Nfreq,self.Narr])
